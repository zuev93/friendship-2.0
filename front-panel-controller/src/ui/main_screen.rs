...
pub fn render(target: &mut impl DrawTarget<Color = Rgb565>, state: &RadioState, error_count: u32) {
    let _ = Rectangle::new(Point::zero(), Size::new(240, 135))
        .into_styled(PrimitiveStyle::with_fill(BLACK))
        .draw(target);

    draw_status_bar(target, state, error_count);
...
fn draw_status_bar(
    ...
    Text::new(bl, Point::new(x, 10), mode_style)
        .draw(target)
        .ok();
    ...
    Text::new(tx_mode, Point::new(x, 10), mode_style)
        .draw(target)
        .ok();
    ...
    Text::new(mode_label, Point::new(mode_x, 10), mode_rx_style)
        .draw(target)
        .ok();

    if error_count > 0 {
        let err_style = MonoTextStyle::new(&FONT_6X10, RED);
        let mut buf: String<8> = String::new();
        let _ = write!(buf, "E:{}", error_count);
        let err_x = mode_x - (buf.len() as i32) * 6 - 4;
        Text::new(&buf, Point::new(err_x, 10), err_style)
            .draw(target)
            .ok();
    }
}
...
fn draw_frequency(target: &mut impl DrawTarget<Color = Rgb565>, frequency: u32) {
    ...
    Text::new(&freq_str, Point::new(freq_x, 40), freq_style)
        .draw(target)
        .ok();

    let unit_style = MonoTextStyle::new(&FONT_6X10, GRAY);
    Text::new("MHz", Point::new(freq_x + text_width + 4, 40), unit_style)
        .draw(target)
        .ok();
}
...
fn draw_info_line(target: &mut impl DrawTarget<Color = Rgb565>, state: &RadioState) {
    ...
    if state.clarifier_mode != 0 {
        let label = if state.clarifier_mode == 1 {
            "RIT"
        } else {
            "XIT"
        };
        ...
        Text::new(&rit_str, Point::new(x, y), style)
            .draw(target)
            .ok();
    ...
    Text::new(&bw_str, Point::new(x, y), style)
        .draw(target)
        .ok();
    ...
    Text::new(&pwr_str, Point::new(x, y), style)
        .draw(target)
        .ok();
...
fn draw_smeter(target: &mut impl DrawTarget<Color = Rgb565>, dbm: i8) {
    ...
    Text::new(&dbm_str, Point::new(168, bar_y + 11), dim_style)
        .draw(target)
        .ok();
    ...
    Text::new(&s_str, Point::new(218, bar_y + 11), readout_style)
        .draw(target)
        .ok();
}
...
fn draw_vol_sql(target: &mut impl DrawTarget<Color = Rgb565>, state: &RadioState) {
    ...
    Text::new("VOL", Point::new(2, y + 10), label_style)
        .draw(target)
        .ok();
    ...
    Text::new("SQL", Point::new(120, y + 10), label_style)
        .draw(target)
        .ok();
